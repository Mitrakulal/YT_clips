...
| _today_ | 4 | ✅ | worker + launchd 24/7 — `stage.py` (idempotent stage machine) + `worker.py` (file-queue loop) net-new; queue/{inbox,active,done,failed}, jobs/, published/, logs/ dirs; LaunchAgent `com.user.shorts.pipeline` (KeepAlive, 30s throttle, logs to logs/worker.{log,err.log}). Verified e2e on Mac (M4, Ollama qwen3:14b local): real YouTube URL `UNP03fDSj1U` → **status done**, all 6 stages green, 2 published captioned clips 1080×1920 (23.6s / 18.6s, ≤60s), captions confirmed via OCR of burned frames. Env fix: stock Homebrew ffmpeg lacks libass → installed **ffmpeg-full** (libass+freetype) + symlinked into PATH; committed code unchanged. |
| _today_ | 5 | ✅ | hardening — `.env`/`.env.example` add `LOCAL_WHISPER_VAD_FILTER=false` + `PIPE_RETRY_LIMIT` knob comment. Model-cache pinning (external `OLLAMA_MODELS`/`HF_HOME` + `.zshrc` export) **cut**: plan marks it optional; this Mac has 70 GB free (not the assumed 256 GB), and repointing `OLLAMA_MODELS` would break the pre-existing Ollama model store. `logs/worker*.log` auto-bounded by launchd ThrottleInterval + KeepAlive restart. |
