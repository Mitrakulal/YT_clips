...
def api_jobs() -> Response:
    ...
    response = jsonify([row_to_job(row) for row in rows])
...
const currentJob={{ job_id|tojson if page=='job' else 'null' }};let selection={num_clips:3,aspect_ratio:'9:16',quality:'1080'};let jobPollTimer=null;let jobPollInFlight=false;
...
async function loadJobs(){let list=document.getElementById('job-list');if(!list)return;try{let r=await fetch('/api/jobs',{cache:'no-store'});if(!r.ok)throw new Error('Could not load jobs');let jobs=await r.json();document.getElementById('job-count').textContent=jobs.length+' job'+(jobs.length===1?'':'s');list.innerHTML=jobs.length?jobs.map(j=>'<a class="job-row" href="/jobs/'+j.id+'"><span class="job-id">#'+j.id.slice(-6)+'</span><span class="job-url" title="'+j.source_url+'">'+j.source_url+'</span><span class="hide-small">'+badge(j.status)+'</span><span class="job-meta hide-small">'+j.clip_count+' clip'+(j.clip_count===1?'':'s')+'</span><span class="job-meta">'+fmtDate(j.created_at)+'</span><span class="arrow">→</span></a>').join(''):'<div class="empty"><strong>No clips in the room yet.</strong>Submit a source above to start your first local job.</div>'}catch(e){console.warn('Local job history refresh failed; retrying automatically.',e)}}
...
function scheduleJobPoll(delay){clearTimeout(jobPollTimer);jobPollTimer=setTimeout(loadJob,delay)}
